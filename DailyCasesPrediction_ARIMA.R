...
# 4. Convert date to a date object and create time series object from new cases data
...
# Extract data of date and new cases from Feb 2020 onwards (2022-12-31) to fit into prediction model
newcase = "New_cases"
date = "Date_reported"
range_row = c(7:1071)
column = c(date, newcase)
# The final data for prediction model
...
cases_ts = ts(df$New_cases, frequency = 12)
...
write.csv(predictions_df, "/Users/Zul/Desktop/cov_predictCase_ARIMAmodel.csv", row.names=TRUE)



#################### PLOT simple black graph
# Plot the predictions with actual dates as x-axis labels
plot(predictions_df$date, predictions_df$cases, type = "l", xlab = "Date", ylab = "New Daily Cases", main = "Predicted Daily Covid-19 Cases for January - March 2023 in Malaysia", xaxt = "n")
...
plot(predictions_df$date, predictions_df$cases, type = "n", xlab = "Date", ylab = "New Daily Cases", main = "Predicted Daily Covid-19 Cases for January - March 2023 in Malaysia", xaxt = "n")
